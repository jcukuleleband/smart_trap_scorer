...
export async function createApp({dataDir=resolve(root,'.data'),code=process.env.DEV_ACCESS_CODE||'FIELD-DEMO'}={}){
 ...
 return http.createServer(async(req,res)=>{
  ...
  try{
   ...
   if(!path.startsWith('/api/')){const allowed=['/','/index.html','/app.js','/queue.js','/style.css','/sw.js','/manifest.webmanifest','/icon.svg'];if(!allowed.includes(path)||!['GET','HEAD'].includes(req.method))fail(404,'Not found');const file=path==='/'?'index.html':path.slice(1);const bytes=await readFile(resolve(root,'public',file));const types={'.html':'text/html','.js':'text/javascript','.css':'text/css','.svg':'image/svg+xml','.webmanifest':'application/manifest+json'};res.writeHead(200,{'Content-Type':types[extname(file)],'Cache-Control':'no-cache'});res.end(req.method==='HEAD'?undefined:bytes);return;}
   const origin=req.headers.origin;
   if(origin&&origin!==`http://${req.headers.host}`)fail(403,'Origin not permitted');
   ...
   if(path==='/api/session'&&req.method==='POST'){
    ...
    const id=randomBytes(32).toString('hex'),csrf=randomBytes(24).toString('hex');sessions.set(id,{csrf,expires:Date.now()+3600000});res.setHeader('Set-Cookie',`fieldbook_session=${id}; HttpOnly; SameSite=Strict; Path=/; Max-Age=3600`);send(200,{event,csrf});return;
   }
   ...
   if(path==='/api/session'&&req.method==='DELETE'){sessions.delete(token);res.setHeader('Set-Cookie','fieldbook_session=; HttpOnly; SameSite=Strict; Path=/; Max-Age=0');send(200,{ok:true});return;}
